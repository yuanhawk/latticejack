...
| Arm64 optimization (B2) | **Two levers tried on real hardware, both honest null results.** Lever 1 (session resumption): ~2%/~0% benefit, not the plan's assumed "near-guaranteed win." Lever 2 (JVM flags): every variant landed within ~1% of baseline on real Arm64 — a promising -15.5% seen locally (Apple Silicon) didn't hold. Bonus finding: JDK 25's built-in ML-KEM (marketed at ~2x BC's speed) only beats BC by ~5-11% on real Arm64 server hardware, and BC is often *faster* at cold start. Consistent pattern: ML-KEM's own crypto cost dominates so heavily on this hardware that every secondary lever tested so far is noise by comparison. See [benchmarks/samples/azure-cobalt100-2vcpu/README.md](benchmarks/samples/azure-cobalt100-2vcpu/README.md) and [benchmarks/mlkem-microbench/README.md](benchmarks/mlkem-microbench/README.md). |
