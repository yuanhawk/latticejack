...
measured on — now backed by a real, KleidiAI-accelerated LLM served
behind this project's actual PQC handshake on that same hardware, not
just the argument (see [benchmarks/ai-inference-pqc/](benchmarks/ai-inference-pqc/README.md)).
Full argument: `WRITEUP.md`'s Project Overview.

<img src="docs/charts/b2-levers.svg" alt="Eight B2 optimization levers vs their own baseline: two null results (session resumption, JVM flags, ~1x), three modest wins (Vector API 1.07x, JDK 25 intrinsic 1.09x, RustCrypto 1.3x), and two large wins (mlkem-native NEON 4.0x, GraalVM native-image 7.9x)" width="620">
...
| Authoring guardrail / CBOM (Component C) | **Done, and executed, not just authored.** [`skills/pqc-authoring/`](skills/pqc-authoring/SKILL.md) — a Claude Code Skill that reviews new/changed TLS code for regressions back to classical-only crypto. Two artifacts: the [worked example](skills/pqc-authoring/examples/worked-example.md) narrates a real regression this project hit once (`SSLContext.getDefault()` silently negotiating classical, no runtime error), authored to explain the skill, not a recorded run. The [executed review transcript](skills/pqc-authoring/examples/executed-review-transcript.md) is the recorded run: three new, never-before-seen snippets reviewed in one live pass, covering three checklist items the worked example doesn't (a keystore-generation bug, a second unwired TLS listener), correctly flagging both real regressions **and** correctly declining to flag a plausible-looking true negative (RSA used for JWT signing, out of the skill's stated scope). Also genuinely installable elsewhere: [`.claude/skills/pqc-authoring/`](.claude/skills/pqc-authoring/SKILL.md) is a generalized, project-agnostic copy — drop its directory into any other repo's `.claude/skills/` and Claude Code auto-discovers it there, no build step (mechanics: [component-c/README.md](component-c/README.md#using-this-skill-in-your-own-project)). [`component-c/cbom/`](component-c/README.md) — `./run cbom {before\|after}` emits a CycloneDX 1.6 CBOM, validated against the real published schema with a committed, reproducible validator (`component-c/cbom/validate_cbom.py`), honestly showing ECDSA-P256 (cert auth) as still-unmigrated in the "after" BOM since that's the actual state of this project's migration scope. |
| Real AI workload behind the PQC handshake | **Done, on real Arm64 hardware, and positively verified — not asserted.** [`benchmarks/ai-inference-pqc/`](benchmarks/ai-inference-pqc/README.md): a quantized LLM (Llama-3.2-1B-Instruct, Q4_0) served by `llama.cpp`'s KleidiAI Arm CPU backend, fronted by this project's real hybrid X25519MLKEM768 handshake, on the same Azure Cobalt 100 VM every other real-hardware number here was measured on. KleidiAI engagement checked at the log and symbol level (`--verbose` kernel-selection lines, non-zero `CPU_KLEIDIAI` weight-buffer size, `nm`), not assumed from "it ran." 3 verified runs (HelloRetryRequest + real model reply each time): handshake ~327ms avg vs. request-to-response ~3065ms avg — handshake cost ~9.4x smaller than the AI inference it fronts. Single client, no concurrency, one small model — see that README's "what this does not establish" section before citing this beyond what it showed. |
| JDK 25 re-baseline of the FFM benchmarks (L5/L7/L8) | **Done — the disclosed cross-JDK confound (JDK 21 FFM vs. JDK 25 BC/JDK25-builtin) does not meaningfully move any of the three ratios.** All three FFM benchmarks recompiled and rerun on JDK 25 (no `--enable-preview` needed — confirms FFM's JDK 22+ finalization), same native libraries, same Azure hardware: total time moved <0.5% for all three vs. their committed JDK 21 numbers. See the "JDK 25 re-baseline" section in [mlkem-ffm-bench](benchmarks/mlkem-ffm-bench/README.md#jdk-25-re-baseline), [mlkem-rust-ffm-bench](benchmarks/mlkem-rust-ffm-bench/README.md#jdk-25-re-baseline), [pqcrypto-ffm-bench](benchmarks/pqcrypto-ffm-bench/README.md#jdk-25-re-baseline). |
| Live demo (judge-triggered, real hardware) | **Deployed and working.** [latticejack.itinerario.io](https://latticejack.itinerario.io) — click Start, a real Azure Cobalt 100 VM boots on demand and streams its actual output to the browser (classical TLS, hybrid PQC, native ML-KEM, real AI request), then deallocates itself. Not a recording. See [`demo/README.md`](demo/README.md) for the architecture and what's been verified on real infrastructure. |
| Demo video | **Live on YouTube:** [youtu.be/E66YU0AVoE8](https://youtu.be/E66YU0AVoE8) — ~2:37, built around the live demo above rather than raw terminal commands. Production account (including two real bugs found and fixed doing it for real): [`docs/video-production/README.md`](docs/video-production/README.md). |
